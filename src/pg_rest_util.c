...
bool
pgrest_util_cmp_sockaddr(struct sockaddr *sa1, socklen_t slen1,
...
in_port_t
pgrest_util_get_port(struct sockaddr *sa)
{
    struct sockaddr_in   *sin;
#ifdef HAVE_IPV6
    struct sockaddr_in6  *sin6;
...
void
pgrest_util_set_port(struct sockaddr *sa, in_port_t port)
{
    struct sockaddr_in   *sin;
#ifdef HAVE_IPV6
    struct sockaddr_in6  *sin6;
...
in_addr_t
pgrest_util_inet_addr(unsigned char *text, size_t len)
{
    unsigned char  *p, c;
    in_addr_t       addr;
...
#ifdef HAVE_IPV6
bool
pgrest_util_inet6_addr(unsigned char *p, size_t len, unsigned char *addr)
{
    ...
    for (/* void */; len; len--) {
        ...
        if (c == '.' && nibbles) {
            ...
            word = pgrest_util_inet_addr(digit, len4 - 1);
            if (word == INADDR_NONE) {
                return false;
            }
...
bool
pgrest_util_resolve_host(pgrest_url_t *u)
{
    char                 *p, *host;
    size_t                len;
    in_port_t             port;
    ...
    for (rp = res; rp != NULL; rp = rp->ai_next) {
        ...
        (void) pgrest_util_inet_ntop((struct sockaddr *) sin6, 
                                     rp->ai_addrlen, 
                                     p,
                                     len, 
                                     true);
...
static bool
pgrest_util_parse_unix(pgrest_url_t *u)
{
#ifdef HAVE_UNIX_SOCKETS
    char                *path;
    size_t               len;
...
static bool
pgrest_util_parse_inet6(pgrest_url_t *u)
{
#ifdef HAVE_IPV6
    ...
    if (!pgrest_util_inet6_addr((unsigned char *)host, 
                                len,
...
static bool 
pgrest_util_parse_inet(pgrest_url_t *u)
{
    char                 *p, *host, *port, *last;
    size_t                len;
    int                   n;
    ...
    sin->sin_addr.s_addr = pgrest_util_inet_addr((unsigned char *)host, len);
...
bool
pgrest_util_parse_url(pgrest_url_t *u)
{
    char   *p;
    size_t  len;
    ...
    if (len >= 5 && strncasecmp(p, (char *) "unix:", 5) == 0) {
        return pgrest_util_parse_unix(u);
    }

    if (len && p[0] == '[') {
        return pgrest_util_parse_inet6(u);
    }

    return pgrest_util_parse_inet(u);
}
